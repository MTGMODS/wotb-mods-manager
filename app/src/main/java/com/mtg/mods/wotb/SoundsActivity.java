...
public class SoundsActivity extends AppCompatActivity {
	...
	private DatabaseReference fsdb = _firebase.getReference("Mods/Sounds");
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.sound, null);
			}
			...
			textview6.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)80, (int)5, 0xFF4CAF50, Color.TRANSPARENT));
			textview4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)80, (int)5, 0xFFF44336, Color.TRANSPARENT));
			if (settings.getString("language", "").contains("english")) {
				textview1.setText("".concat(_data.get((int)_position).get("mod").toString()));
				textview2.setText("Mod author: ".concat(_data.get((int)_position).get("author").toString()));
				textview5.setText("Adden/updated: ".concat(_data.get((int)_position).get("date").toString()));
				textview6.setText("  Download  ");
			}
			else {
				textview1.setText("".concat(_data.get((int)_position).get("mod").toString()));
				textview2.setText("Автор мода: ".concat(_data.get((int)_position).get("author").toString()));
				textview5.setText("Добавлено/обновлено: ".concat(_data.get((int)_position).get("date").toString()));
			}
			...
			textview4.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					...
					i.setData(Uri.parse(_data.get((int)_position).get("video").toString()));
					startActivity(i);
				}
			});
